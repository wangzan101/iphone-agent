...
| 利用布局打开 App | 已实现候选布局查找、身份检查与降级路径。 |
| 跨任务记忆和技能 | 已有独立实现，后续继续完善整合。 |
| 更完整的屏幕与转移知识 | 后续数字孪生方向。 |
...
**当前 scan 会回到主屏第一页并记录这一页，并不是扫描整台手机或所有 App。** 任务中的后续观察也可以刷新布局信息。
...
首版 Public 候选在独立的 macOS / Python 3.13 环境中通过了 **1,344 个自动化测试**。这是回归测试结果，不是真机任务成功率。[验证说明](docs/evaluation.md) · [架构设计](docs/architecture.md)
